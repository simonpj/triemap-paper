...
input.  The latter returns a bag of \lstiinline{(PatSubst, (PatKeyMap, v))}
...
Now we can return to the recursive function that does all the work: \lstinline{lkT}:
\begin{code}
lkT :: forall v. ExprS -> (PatSubst, MExprSMap v) -> Bag (PatSubst, v)
lkT e (psubst, mt)
...
\end{code}
The bag of results is the union of three possibilities:
\begin{itmemize}
  \item \lstinline{pat_var_bndr}: we consult the \lstinline{mm_pvar} `at least one of the patterns in this trie has a pattern binder $\pv{}$ at this spot.  In that case
