...
$$
   f~\#1~\#2~True      \qquad and \qquad  f~\#1~\#2~False
$$
By numbering the variables left-to-right, we ensure that they ``line up''.

What if the variable occurs more than once? Suppose we are matching
...
be canonicalised to $(f\,\#1\,\$1\,\$1)$, where the first (or binding) occurrence
is denoted $\#i$ and subsequent (bound) occurrences are denoted $\$i$.
...
of a pattern variable $\#i$, and the fourth with a bound occurrence of
a pattern variable $\$i$.  Quite 

\begin{code}
lkT :: ExprS -> (PatSubst, MExprSMap v) -> Bag (PatSubst, v)
